...
Задана строка S, состоящая из прописных букв латинского алфавита, и символ С через пробел. Длина строки не превышает 100 символов. Гарантируется, что символ С встречается в строке S.

Выходные данные

Выведите единственное число ходов, которое потребуется, чтобы переместить все символы с начало строки
    ...
    for j in range(len(S)):
        if S[j] == C:
            while j > i:
                ...
                j -= 1
                moves += 1
                print(f"На шаг №{moves:3} строка выглядит так: {''.join(S)}")
            i += 1
...
print(f"Total moves required: {move_count(S, C)}")
